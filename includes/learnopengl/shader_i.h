...
public:
		...
		const char* controlPath = nullptr, const char* evaluationPath = nullptr)
	{
		...
		{
			// open files
			careOpen(vShaderFile, vertexPath);
			//safeOpen(vShaderFile, vertexPath);
			careOpen(fShaderFile, fragmentPath);
			...
			{
				careOpen(gShaderFile, geometryPath);
				std::stringstream gShaderStream;
				gShaderStream << gShaderFile.rdbuf();
				gShaderFile.close();
				geometryCode = gShaderStream.str();
			}
			// if control shader path is present, also load a geometry shader
			if (controlPath != nullptr)
			{
				careOpen(cShaderFile, controlPath);
				std::stringstream cShaderStream;
				cShaderStream << cShaderFile.rdbuf();
				cShaderFile.close();
				controlCode = cShaderStream.str();
			}
			// if evaluation shader path is present, also load a geometry shader
			if (evaluationPath != nullptr)
			{
				careOpen(eShaderFile, evaluationPath);
		...
		// if control shader is given, compile geometry shader
		...
		// if evaluation shader is given, compile geometry shader
